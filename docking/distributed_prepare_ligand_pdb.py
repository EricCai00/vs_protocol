...
def distributed_prepare_ligand_pdb(input_dir, output_dir, threads, force, verbose, blocked_nodes):
    ...
    input_dir_abs = Path(input_dir).resolve()
    output_dir_abs = Path(output_dir).resolve()

    if not input_dir_abs.is_dir():
        print(f"Error: Input dir not found: {input_dir_abs}", file=sys.stderr)
        sys.exit(1)

    for dir_path, dir_name_desc in [(output_dir_abs, "Final PDBQT output directory")]:
    ...
    cd {input_dir_abs}
    {sq_python2_exe} {sq_prepare_ligand4_script} -l {{}} -o {sq_pdbqt_target_dir_path}/{{/.}}.pdbqt
    """
    ...
    p1 = subprocess.Popen(['find', '.', '-name', '*.pdb'], stdout=subprocess.PIPE)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_dir", required=True, help="Input directory containing PDB file.")
    ...
    distributed_prepare_ligand(args.input_dir, args.output_dir, args.threads, args.force, args.verbose, '')
