...
CODE_PATH=/public/home/caiyi/code
DD_CODE_PATH=/public/home/caiyi/code/vs_integrated/dd_code
...
IFS=$' ' read -r -a gpu_array <<< $($PYTHON3 $CODE_PATH/vs_integrated/parse_nvidia_smi.py $auto_threads)
actual_threads=${#gpu_array[@]}

echo PREDICTING USING $actual_threads GPUS: ${gpu_array[@]}, jobs_per_gpu=$jobs_per_gpu
...
while [ ! ${#job_array[@]} -eq 0 ]; do
    ...
    sleep 1
    {
        ...
        gpu_id=${index#*:}
        time1=$(date +%s.%2N)
        echo $wd/simple_job_predictions/simple_job_$job_idx.sh
        ssh gpu$node_id "cd $DD_CODE_PATH; export LD_LIBRARY_PATH=$LIB_PATH; " \
            "CUDA_VISIBLE_DEVICES=$gpu_id bash $wd/simple_job_predictions/simple_job_$job_idx.sh"
        time2=$(date +%s.%2N)
        time_delta=$(echo $time2 $time1 | awk '{print($1-$2)}')
        echo job $job_idx: $time_delta $(date +%H:%M:%S.%3N) >> $wd/inferring_time.txt
    } &
    i=$((i+1))
    done
