    ...
    with open(fname,'r') as ref:
        for line in ref:
            ...
            if (tmp[0] in Oversampled_zid.keys()) and (type(Oversampled_zid[tmp[0]]) != np.ndarray):
                train_set = np.zeros([1,1024], dtype=np.uint8)
                on_bit_vector = tmp[1:]
...
print('HITS:', (y_valid_first.r_i_docking_score < cf).values.sum(), 
...
if SMILES:
    ...
    Oversampled_X_train = np.zeros([sample_size*2, len(list(Oversampled_zid.values())[0][0])], dtype=np.uint8)
    print(len(list(Oversampled_zid.values())[0]))
else:
    Oversampled_X_train = np.zeros([sample_size*2, 1024], dtype=np.uint8)
...
ct = 0
Oversampled_y_train = np.zeros([sample_size*2, 1], dtype=bool)
...
# np.save('prediction_valid', prediction_valid)
